...
namespace ft	{
	...
	class mapBase	{
		...
		public:
			...
			mapBase( void ) : head(NULL), tail(NULL), tailStorage(NULL), alloc(allocator_type()) {

				if (DEBUG_MODE >= 2) std::cout << "CONSTRUCTOR --> default " << __func__ << std::endl;
			}

			mapBase( allocator_type const & userAlloc ) : head(NULL), tail(NULL), tailStorage(NULL), alloc(userAlloc) {

				if (DEBUG_MODE >= 2) std::cout << "CONSTRUCTOR --> defaut with alloc " << __func__ << std::endl;
			}

			mapBase( size_t n ) : head(NULL), tail(NULL), tailStorage(NULL), alloc(allocator_type()) {

				initStorage(n + (n>>1));
				if (DEBUG_MODE >= 2) std::cout << "CONSTRUCTOR --> fill " << __func__ << std::endl;
			}

			mapBase( size_t n, allocator_type const & userAlloc ) : head(NULL), tail(NULL), tailStorage(NULL), alloc(userAlloc) {
	...
	class map : protected mapBase<T, T_alloc> {

		private:
			typedef mapBase<T, T_alloc>					mapBase;
		public:
			typedef T										value_type;
   			typedef typename mapBase::allocator_type		allocator_type;
			typedef typename T_alloc::reference				reference;
			typedef typename T_alloc::const_reference		const_reference;
			typedef typename T_alloc::pointer				pointer;
			typedef typename T_alloc::const_pointer			const_pointer;
...
.******************************************************************************/
		...
		public:
			...
			*/
			explicit map( allocator_type const & userAlloc = allocator_type() ) : mapBase(userAlloc) {
			...
			*/
			explicit map( size_type n, value_type const & val = value_type(),
				allocator_type const & userAlloc = allocator_type() ) : mapBase(n, userAlloc)	{
			...
			*/
			explicit map( map const & src ) : mapBase(src.size()) {
